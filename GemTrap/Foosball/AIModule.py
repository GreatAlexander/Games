...
#==============================================================================
	...
	def player_is_displaced(self):
		...
		or abs(self.curr_pos[1]-self.start_pos[1]) <= self.move_tolerance):
	...
	def behaviour_goto_ball(self):
		...
		self.target_angle = self.calc_angle_to_target_absolute(self.vector_to_target)
		self.target_approach_angle = 4
		self.target_approach_distance = AGENT_RADIUS + 20
	...
	def behaviour_goto_start(self):
		...
		self.target_approach_distance = 10
...
#		self.target_angle = self.curr_angle
	...
	def calc_angle_to_target_absolute(self, vect):
		...
		if xmod == 0:
			...
			target_angle = np.rad2deg(np.arctan(ymod/xmod))		
		
		if vect[0] >= 0 and vect[1] <= 0:
			absolute_target_angle = 90 - target_angle	
		elif vect[0] >= 0 and vect[1] >= 0:
			absolute_target_angle = 90 + target_angle
		elif vect[0] <= 0 and vect[1] >= 0:
			absolute_target_angle = 270 - target_angle	
		elif vect[0] <= 0 and vect[1] <= 0:
			absolute_target_angle = 270 + target_angle	
		...
		if hyp == 0:
			...
			return absolute_target_angle
		
	def path_planning(self):
		
		self.distance_to_target = self.calc_distance_to_target(self.vector_to_target)
		self.angle_to_target_absolute = self.calc_angle_to_target_absolute(self.vector_to_target)	
